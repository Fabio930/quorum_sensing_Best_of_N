                                        ...
                                        for mt in $message_timeout; do
                                            for ms in $messages_per_step; do
                                                for mh in $message_hops; do
                                                    config=`printf 'config__Nrobots_%s__model_%s__distribution_%s__rebroadcast_%s__Noptions_%s__Rtype_%s__Rval_%s__eta_%s__minBuff_%s__msgTime_%s__msgXstep_%s__msgHops_%s.xml' $agents $mdl $distr $rebrcts $options $type $val $et $qlm $mt $ms $mh`
                                                    ...
                                                    sed -i "s|__COMPUTING__|$cmpt|g" $config
                                                    sed -i "s|__DISTRIBUTION__|$distr|g" $config
